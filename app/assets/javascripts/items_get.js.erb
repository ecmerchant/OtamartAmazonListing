...
var handsontable = new Handsontable(container, {
  ...
  data: mydata,
  rowHeaders: true,
  colHeaders: ["オタマート商品URL","商品タイトル","商品ID","販売価格","タグ","状態","出品者","配送日の目安","画像1","画像2","画像3"],
  maxCols: maxColnum,
  maxRows: maxRownum,
  manualColumnResize: true,
  autoColumnSize: false,
  colWidths:[200,300,150,120,220,120,120,120,120,120,120]
  //columns: colOption
});
  ...
  $.ajax({
    ...
    success: function (myData) {
      ...
      if(counter > urlnum){
        ...
        for(var p = 0; p < orgData.length; p++){
          if(orgData[p][1] != "【商品ページなし】"){
            new_data[k] = orgData[p];
            k++;
          }
          if(orgData[p][0] == ""){
            break;
        ...
        var emp_data = ["","","","","","","","","","",""];
        for(var p = k; p < orgData.length; p++){
          new_data[p] = emp_data;
        }
